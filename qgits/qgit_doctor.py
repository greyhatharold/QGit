...
from .qgit_errors import (
    ...
    GitNetworkError, FileOperationError, format_error
)
from .qgit_utils import format_size
...
class RepositoryDoctor:
    ...
    def add_issue(self, category: str, severity: str, message: str, fix_command: Optional[str] = None) -> None:
                ...
                if '=' in line:
                    ...
                    if key in seen_keys:
                        self.add_issue(
                            ...
                            f"# Manual fix required - check .git/config and ~/.gitconfig"
                        )
                    seen_keys[key] = value

            return len([i for i in self.issues if i['category'] == 'config' and i['severity'] == 'critical']) == 0
    ...
    def check_remote_connection(self) -> bool:
        ...
        try:
            ...
            return len([i for i in self.issues if i['category'] == 'remote' and i['severity'] == 'critical']) == 0
    ...
    def check_large_files(self, size_threshold_mb: int = 100) -> bool:
        ...
        try:
            ...
            sizes = GitCommand.run("git cat-file --batch-check='%(objectname) %(objecttype) %(objectsize) %(rest)'"
                                 " --batch-all-objects").split('\n')
            ...
            for size_line in sizes:
                ...
                if len(parts) >= 3 and parts[1] == 'blob':
                    ...
                    if size_mb > size_threshold_mb:
                        filename = next((line.split()[1] for line in objects if line.startswith(parts[0])), "unknown")
            ...
            self.add_issue('storage', 'warning', f"Error checking large files: {str(e)}")
            return False
    ...
    def check_branch_status(self) -> bool:
        ...
        try:
            ...
            try:
                behind_ahead = GitCommand.run(f"git rev-list --left-right --count origin/{current}...HEAD").split()
            ...
            self.add_issue('branch', 'critical', f"Error checking branch status: {str(e)}")
            return False
    ...
    def check_gitignore(self) -> bool:
        ...
        try:
            ...
            self.add_issue('gitignore', 'warning', f"Error checking .gitignore: {str(e)}")
            return False
    ...
    def check_lfs_status(self) -> bool:
        ...
        try:
            ...
            large_files = []
            binary_extensions = {'.zip', '.pdf', '.png', '.jpg', '.jpeg', '.gif', '.mp4', '.mov', '.bin'}
            ...
            if lfs_installed:
                ...
                try:
                    ...
                    self.add_issue('lfs', 'warning', f"Error checking LFS files: {str(e)}")
    ...
    def check_commit_history(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                if conflicts:
                    self.add_issue(
                        ...
                        "# Manually resolve conflicts in: " + conflicts.replace('\n', ', ')
                    )
            except GitCommandError:
            ...
            large_commits = []
            log = GitCommand.run("git log --pretty=format:'%h %ad %s' --date=short --numstat")
            current_commit = None
            current_changes = 0
            ...
            self.add_issue('history', 'warning', f"Error checking commit history: {str(e)}")
            return False
        ...
        try:
            ...
            if os.path.exists('.gitmodules'):
                try:
                    ...
                    self.add_issue('submodules', 'critical', f"Error checking submodules: {str(e)}")
                    return False

            return True

        except Exception as e:
            self.add_issue('submodules', 'warning', f"Error checking submodules: {str(e)}")
            return False
    ...
    def run_all_checks(self) -> bool:
        ...
        for check in checks:
            try:
                ...
                self.add_issue('system', 'critical', f"Error running {check.__name__}: {str(e)}")
                all_critical_passed = False
        ...
        for issue in self.issues:
            if issue['fix_command'] and not issue['fix_command'].startswith('#'):
                try:
                    ...
                    self.fixes_applied.append(f"Fixed {issue['category']}: {issue['message']}")
                    fixes_applied += 1
