...
class QGitLogger:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self.resource_manager = get_resource_manager(Path(os.path.dirname(self.db_path)))
    ...
    def _write_entries_to_db(self, entries: List[LogEntry]):
        ...
        with self._get_db() as conn:
            conn.executemany(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                [(e.timestamp, e.level, e.command, e.message,
                  json.dumps(e.metadata), e.status, e.duration)
                 for e in entries]
            )

    def log(self, level: str, command: str, message: str,
        ...
        entry = LogEntry(
            ...
            command=command,
            message=message,
            metadata=self.resource_manager.memory_manager.get_cached_data(str(metadata)) or metadata or {},
            status=status,
            duration=duration
        )
    ...
    def get_logs(self, limit: int = 100, level: Optional[str] = None,
                command: Optional[str] = None, start_date: Optional[str] = None,
