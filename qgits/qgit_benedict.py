...
from .qgit_errors import (
    GitCommandError, GitStateError, GitRepositoryError,
    FileOperationError, format_error
)
...
def scan_repository(directory: str = '.', batch_size: int = 1000) -> Tuple[Dict[str, List[Dict[str, Any]]], int]:
    ...
    try:
        ...
        print("\r" + _format_success("Scan completed successfully!") + " " * 50)  # Clear spinner line
        ...
        print("\r" + _format_error(f"Scan failed: {str(e)}") + " " * 50)  # Clear spinner line
...
def _display_scan_summary(results: Dict[str, List[Dict[str, Any]]], total_files: int) -> None:
    ...
    print(f"Total risky files found: {COLORS['WARNING' if total_risky > 0 else 'GREEN']}{total_risky:,}{COLORS['ENDC']}")
    print(f"Total size of risky files: {COLORS['BOLD']}{format_size(total_size)}{COLORS['ENDC']}")
    ...
    for category, files in results.items():
        ...
        print(_format_subheader(f"{emoji} {category.title()} ({len(files)} files, {format_size(total_cat_size)})"))
...
def _display_category_details(files: List[Dict[str, Any]]) -> None:
    ...
    for pattern, group_files in pattern_groups.items():
        ...
        print(f"     Found {COLORS['BOLD']}{len(group_files)}{COLORS['ENDC']} files ({format_size(group_size)})")
        
        # Show top 5 largest files
        sorted_files = sorted(group_files, key=lambda x: x['size'], reverse=True)
        for file_info in sorted_files[:5]:
            size_str = format_size(file_info['size'])
            print(f"     • {COLORS['BLUE']}{file_info['path']}{COLORS['ENDC']} ({size_str})")
        
        if len(group_files) > 5:
            print(f"     {COLORS['WARNING']}... and {len(group_files) - 5} more files{COLORS['ENDC']}")

def update_gitignore(scan_results: Dict[str, List[Dict[str, Any]]], auto_commit: bool = False) -> bool:
    ...
    except (GitCommandError, GitStateError, GitRepositoryError, FileOperationError) as e:
...
def _commit_gitignore_changes() -> bool:
    ...
    try:
        ...
        GitCommand.commit("🔒 Update .gitignore with security patterns from qgit benedict")
...
def _find_problematic_files(tracked_files: List[str], patterns: List[str]) -> Dict[str, Dict[str, Any]]:
...
def reverse_tracking(file_patterns: List[str] = None) -> bool:
    ...
    response = input("\nDo you want to untrack these files and update .gitignore? (y/N): ").lower()
    if response != 'y':
...
def expel() -> bool:
    ...
    try:
        ...
        for i in range(0, len(tracked_files), 100):
            ...
            print(f"\rUntracking batch {current_batch}/{total_batches} ({len(batch)} files)...", end='', flush=True)
            files_str = ' '.join(f'"{f}"' for f in batch)
            GitCommand.run(f"git rm -r --cached {files_str}")
        ...
        print(f"1. {COLORS['CYAN']}Update your .gitignore to prevent re-tracking files{COLORS['ENDC']}")
        print(f"2. {COLORS['CYAN']}Force push with: git push origin --force --all{COLORS['ENDC']}")
        return True
...
def _untrack_all_files(files: List[str]) -> None:
    ...
    print(_format_success("Committing changes..."))
    GitCommand.commit("Untrack all files via qgit expel")
    
    print("\n" + _format_success("All files have been untracked successfully!"))
    print(_format_warning("\nTo complete the process:"))
    print(f"1. {COLORS['CYAN']}Update your .gitignore to prevent re-tracking{COLORS['ENDC']}")
    print(f"2. {COLORS['CYAN']}Force push with: git push origin --force --all{COLORS['ENDC']}") 