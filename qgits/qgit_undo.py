...
from .qgit_snapshot import create_snapshot
...
def analyze_operations(operations: List[str]) -> List[Dict[str, Any]]:
    ...
    for op in operations:
        ...
        try:
            ...
            remote_branches = GitCommand.run(f"git branch -r --contains {commit_hash}").strip()
            ...
            operation_info = {
                ...
                'branches_affected': remote_branches.split('\n') if remote_branches else [],
                'timestamp': GitCommand.get_commit_info(commit_hash)['timestamp'].isoformat()
            }
            ...
            print(f"Warning: Could not analyze operation {commit_hash}: {format_error(e)}")
            continue
...
def choose_operations_interactively(operations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def execute_undo_operations(operations: List[Dict[str, Any]], reset_type: str = "--mixed") -> bool:
...
def show_completion_status(operations: List[Dict[str, Any]]) -> None:
    ...
    try:
        ...
        if current_branch != 'HEAD':
            try:
                behind_ahead = GitCommand.run(f"git rev-list --left-right --count origin/{current_branch}...HEAD").split()
                if behind_ahead and len(behind_ahead) == 2:
                    ...
                    if ahead > 0:
                        ...
                        print("  To push these changes: git push --force-with-lease origin " + current_branch)
                    if behind > 0:
...
def undo_operation(steps: int = 1, force: bool = False, dry_run: bool = False,
                  no_backup: bool = False, interactive: bool = False,
                  keep_changes: bool = False, remote_safe: bool = False) -> bool:
    ...
    try:
        ...
        if status and not force:
            ...
            response = input("Would you like to create a snapshot before proceeding? (Y/n): ").lower()
            if response != 'n':
                if not create_snapshot(message=f"Backup before undo operation"):
