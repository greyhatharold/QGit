...
from .qgit_errors import (
    GitCommandError, GitStateError, GitRepositoryError, format_error
)

def create_snapshot(message: Optional[str] = None, no_tag: bool = False, 
                   push: bool = False, stash: bool = False, branch: Optional[str] = None, 
                   expire_days: Optional[int] = None, include_untracked: bool = False) -> bool:
    ...
    try:
        ...
        if not no_tag:
            ...
            tag_message = None
            if expire_days:
                expiry_date = (datetime.now() + timedelta(days=expire_days)).strftime("%Y-%m-%d")
    ...
    try:
        ...
        for tag in tags:
            try:
                ...
                print(f"  Message: {commit_info['message'].split(':')[1].split('(')[0].strip()}")
                if expiry_info:
                    print(f"  {expiry_info}")
        ...
        for tag in tags:
            try:
                ...
                try:
                    expiry = GitCommand.run(f"git tag -l --format='%(contents)' {tag}")
                    if "Expires:" in expiry:
                        expiry_date = datetime.strptime(expiry.split("Expires:")[1].strip(), '%Y-%m-%d')
...
def restore_snapshot(snapshot_id: str) -> bool:
    ...
    try:
        ...
        tag = f"snapshot/{snapshot_id}" if not snapshot_id.startswith("snapshot/") else snapshot_id
        ...
        if status:
            ...
            response = input("Would you like to stash them before proceeding? (Y/n): ").lower()
            if response != 'n':
        ...
        try:
            ...
            if "Expires:" in expiry:
                ...
                if expiry_date <= datetime.now():
                    ...
                    print("You may want to create a new snapshot or commit these changes permanently.")
        except GitCommandError:
            pass
